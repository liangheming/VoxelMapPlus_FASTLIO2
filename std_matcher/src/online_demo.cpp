...
std::queue<pcl::PointCloud<pcl::PointXYZI>::Ptr> laser_buffer;
std::queue<Eigen::Affine3d> odom_buffer;
...
void odomCloudCB(const interface::PointCloudWithOdom::ConstPtr msg)
{
  ...
  laser_buffer.push(cloud);
  odom_buffer.push(pose);
}
...
bool syncPackages(PointCloud::Ptr &cloud, Eigen::Affine3d &pose)
{
  ...
  if (laser_buffer.empty() || odom_buffer.empty())
    return false;
  pcl::copyPointCloud(*laser_buffer.front(), *cloud);
  pose = odom_buffer.front();
  laser_buffer.pop();
  odom_buffer.pop();
  return true;
}
    ...
    const std::vector<Eigen::Affine3d> &key_pose_vec)
{
  ...
  markerNode.header.frame_id = "map"; // map
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher pubLoopConstraintEdge =
      nh.advertise<visualization_msgs::MarkerArray>("/loop_closure_constraints",
                                                    10);
  ros::Subscriber odomSub = nh.subscribe<interface::PointCloudWithOdom>("/lio_node/cloud_with_odom", 100, odomCloudCB);
    ...
    if (syncPackages(current_cloud_body, pose))
    {
      ...
      *key_cloud += *current_cloud_world;
      ...
      if (cloudInd % config_setting.sub_frame_num_ == 0 && cloudInd != 0)
      {
        ...
        ROS_INFO("ID: %lu  FEATRUE SIZE: %lu CLOUD SIZE: %lu", cloudInd, stds_vec.size(), key_cloud->size());
        ...
        if (search_result.first > 0)
        {
          ...
          //           << std::endl;
          ROS_WARN("FIND MATCHED LOOP! CURRENT_ID: %d, LOOP_ID: %d MATCH SCORE: %.4f", std_manager->current_frame_id_, search_result.first, search_result.second);
