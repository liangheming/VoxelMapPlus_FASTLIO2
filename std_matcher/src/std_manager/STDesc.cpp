...
void read_parameters(ros::NodeHandle &nh, ConfigSetting &config_setting)
{
  ...
  // pre-preocess
  nh.param<double>("ds_size", config_setting.ds_size_, 0.25);
  ...
  nh.param<double>("voxel_size", config_setting.voxel_size_, 1.0);
  nh.param<int>("voxel_init_num", config_setting.voxel_init_num_, 10);
  nh.param<double>("proj_image_resolution", config_setting.proj_image_resolution_, 0.25);
  nh.param<double>("proj_dis_min", config_setting.proj_dis_min_, 0.0);
  nh.param<double>("proj_dis_max", config_setting.proj_dis_max_, 5.0);
  nh.param<double>("corner_thre", config_setting.corner_thre_, 10.0);

  // std descriptor
  nh.param<int>("descriptor_near_num", config_setting.descriptor_near_num_, 15);
  nh.param<double>("descriptor_min_len", config_setting.descriptor_min_len_, 2.0);
  nh.param<double>("descriptor_max_len", config_setting.descriptor_max_len_, 30.0);
  ...
  nh.param<double>("vertex_diff_threshold",
                   config_setting.vertex_diff_threshold_, 0.7);
    ...
    std::vector<STDesc> &stds_vec)
{
  ...
  // std::cout << "voxel size: " << voxel_map.size() << " plane : " << plane_count  << std::endl;
...
/**
 * 将所有被判定为非平面的节点向附近的平面进行投影
 * 首先不仅仅使用单个非平面节点，而是使用一圈非平面节点，应该是为了一些避免voxel cut导致的非一致性
 */
...
/**
 * 这里进行图像上的nms,但是实际上最后保留的是图像平面上的点，并不是原始的点，稠密的角点
 */
    ...
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr &corner_points)
{
  ...
  for (size_t i = 0; i < proj_points.size(); i++)
  {
    ...
    // 感觉没什么意义，绕了一大圈结果不如直接投影
    // x +  A (A(x-a) + B(y-b) + C(z-c))
    // y +  B (A(x-a) + B(y-b) + C(z-c))
    // z +  C (A(x-a) + B(y-b) + C(z-c))

    // -ABy  -ACz -AD + x - x*AA = x - A(Ax+By+Cz+D)
    // ((x,y,z) - NORM * (A,B,C)).(1,2,3)
  ...
  for (size_t i = 0; i < point_list_2d.size(); i++)
  {
    ...
    // std::cout << point_list_2d[i].transpose() << ":" << x_index << ":" << y_index << ":" << resolution << ":" << min_x << ":" << min_y << std::endl;
  }
       ...
       x_segment_index++)
  {
         ...
         y_segment_index++)
    {
      ...
      {
        // std::cout << x_segment_index << ":" << y_segment_index << ":" << max_gradient_x_index << ":" << max_gradient_y_index << ":" << max_gradient << std::endl;
    ...
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr &corner_points)
{
  ...
  // std::cout << "nms: " << prepare_key_cloud->size() << " range: " << radius << std::endl;
